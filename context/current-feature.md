...
- Create StockSupply list page with data table (search, category filter, low stock highlight)
- Implement create/edit form for StockSupplies with react-hook-form + zod
- Add delete confirmation dialog (soft-delete with "Deactivate" wording)
- Wire up IPC handlers and preload methods for StockSupplies
- Add TypeScript types for StockSupply, StockSupplyCreateData, StockSupplyUpdateData
- Display category relation in table and form
- Add routes under /admin/manager/stock-supplies in App.tsx

## Notes

- Backend fully ready: all 6 endpoints exist (GET list, GET by id, POST, PUT, DELETE soft-delete, GET categories)
- Backend routes: /api/stock-supplies and /api/stock-supply-categories
- Follow Phase 1 patterns (StockSupplyCategory CRUD) for consistency
- Use shadcn/ui primitives: Card, Button, Input, Textarea, Label, Select, Dialog
- Form pattern from MenuForm.tsx: react-hook-form + zodResolver
- Soft-delete only: DELETE sets isActive=false, supply hidden from list
- Slug auto-generated by backend from name
- Unit enum: KG, G, L, ML, PCS (matches backend ItemUnit)
- Decimal display: format currentStock and reorderLevel to 2 decimal places
- Low stock indicator: highlight rows where currentStock <= reorderLevel
- Category dropdown: fetch from /api/stock-supply-categories on form mount
- Inactive supplies hidden: GET only returns isActive=true supplies
- Spec: context/features/frontend/procurement-phase-2-stock-supplies.md
