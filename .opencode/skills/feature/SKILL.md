...
| **start** | Begin implementation, create branch `feature/<layer>/<task>` |
...
4. **Implement** — Check `## Platform` in `current-feature.md` and use the correct model:
   - **If Platform is `frontend`** → run `ollama run deepseek-coder:latest` for ALL React/TSX/Tailwind/shadcn components in `desktop/ui/`
   - **If Platform is `backend`** → run `ollama run qwen2.5-coder:7b` for Express/Prisma logic in `backend/`
