...
```
Screenshot → qwen2.5vl:3b → design spec → deepseek-coder:latest → frontend code
              (vision)                  (frontend gen)
                                       → qwen2.5-coder:7b  → backend code
                                                              (backend gen)
```

> **Note:** qwen2.5vl:3b only does vision analysis. **It does NOT generate code.** All frontend code generation is handled by `deepseek-coder:latest`. All backend code generation is handled by `qwen2.5-coder:7b`.
